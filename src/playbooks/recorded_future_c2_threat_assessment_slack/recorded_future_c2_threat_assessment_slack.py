"""
This playbook has been created in connection with the Recorded Future App for Phantom version 3.1
to show how the action &quot;Threat Assessment&quot; can be used in a playbook which acts on a
event that have lists of IOCs stored as artifacts. \n\nContext is a required parameter for the
new Threat Assessment action. A list of the possible contexts can be obtained through the other new
action &quot;list contexts&quot; .  The parameter can make use of an artifact but this playbook
works solely on C2.\n\nThe playbook starts by marking the event as &#39;Open&#39; before obtaining
the assessment results from Recorded Future. If the artifacts are deemed malicious, the user gets
a choice of whether to promote the event to a case, otherwise the event is closed.
"""
...
def decision_1(
    ...
    **kwargs,
    ...
    found_match_1 = phantom.decision(
        container=container,
        conditions=[["c2_threat_assessment:action_result.data.*.verdict", "==", True]],
    )
...
def send_slack_message(
    ...
    **kwargs,
    ...
    phantom.act(
        ...
        assets=["slack"],
        callback=set_severity_high,
    )

    return


def set_severity_high(
    action=None,
    success=None,
    container=None,
    results=None,
