...
def splunksearch_for_ip(
    ...
    phantom.act(
        "run query",
        parameters=parameters,
        assets=["splunk", "splunk"],
        callback=splunksearch_for_ip_callback,
        name="splunksearch_for_ip",
    )

    return


def splunksearch_for_ip_callback(
    action=None,
    success=None,
    container=None,
    results=None,
...
def recorded_future_threat_hunting_block_ip(
    action=None,
    success=None,
    container=None,
    results=None,
...
{0}"""
    ...
    phantom.prompt2(
        ...
        message=message,
        respond_in_mins=30,
        name="recorded_future_threat_hunting_block_ip",
        parameters=parameters,
        response_types=response_types,
        callback=check_prompt,
    )
...
def add_ip_to_block_list(
    ...
    results_data_1 = phantom.collect2(
        container=container,
        datapath=["splunksearch_for_ip:action_result.data.*.IP"],
        action_results=results,
    )
...
def spunksearch_for_domains(
    ...
    phantom.act(
        ...
        parameters=parameters,
        assets=["splunk", "splunk"],
        callback=join_check_for_results,
        name="spunksearch_for_domains",
    )
...
def ip_intelligence_1(
    ...
    phantom.act(
        ...
        callback=ip_intelligence_1_callback,
...
def ip_intelligence_1_callback(
    action=None,
    success=None,
    container=None,
    results=None,
...
def extract_ip_addresses(
    ...
    results_data_1 = phantom.collect2(
        container=container,
        datapath=["ip_intelligence_1:action_result.data.*.relatedEntities"],
        action_results=results,
    )
    results_item_1_0 = [item[0] for item in results_data_1]
    ...
    tmplist = [
        line for line in results_data_1[0][0] if line["type"] == input_parameter_0
    ]
    if tmplist:
        tmplist2 = [
            [
                ren["entity"]["name"]
                for ren in line["entities"]
    ...
    phantom.debug(
        "Filter_on_RelatedIpAddress_and_Count relatedList: %s"
        % extract_ip_addresses__relatedList
    )
    phantom.debug(
        "Filter_on_RelatedIpAddress_and_Count resultCount: %d"
        % extract_ip_addresses__resultCount
    )
    ...
    phantom.save_run_data(
        key="extract_ip_addresses:relatedList",
        value=json.dumps(extract_ip_addresses__relatedList),
    )
    phantom.save_run_data(
        key="extract_ip_addresses:resultCount",
        value=json.dumps(extract_ip_addresses__resultCount),
    )
...
def extract_domains(
    ...
    phantom.debug(
        "Filter_on_RelatedDomain_and_count RelatedList: %s"
        % extract_domains__RelatedList
    )
    phantom.debug(
        "Filter_on_RelatedDomain_and_count ResultCount: %d"
        % extract_domains__ResultCount
    )
    ...
    phantom.save_run_data(
        key="extract_domains:RelatedList",
        value=json.dumps(extract_domains__RelatedList),
    )
    phantom.save_run_data(
        key="extract_domains:ResultCount",
        value=json.dumps(extract_domains__ResultCount),
    )
...
def extract_vulnerabilities(
    ...
    if tmplist:
        ...
        if tmplist2:
            ...
            extract_vulnerabilities__resultCount = len(
                extract_vulnerabilities__relatedList
            )
    phantom.debug(
        "Filter_RelatedVulns_and_count relatedList: %s"
        % extract_vulnerabilities__relatedList
    )
    phantom.debug(
        "Filter_RelatedVulns_and_count resultCount: %d"
        % extract_vulnerabilities__resultCount
    )
    ...
    phantom.save_run_data(
        key="extract_vulnerabilities:relatedList",
        value=json.dumps(extract_vulnerabilities__relatedList),
    )
    phantom.save_run_data(
        key="extract_vulnerabilities:resultCount",
        value=json.dumps(extract_vulnerabilities__resultCount),
    )
...
def vulnerability_search_string(
    action=None,
    success=None,
    container=None,
    results=None,
    ...
    phantom.format(
        ...
        template=template,
        parameters=parameters,
        name="vulnerability_search_string",
    )
...
def extract_file_hashes(
    ...
    phantom.debug(
        "Filter_on_RelatedHash_and_count relatedList: %s"
        % extract_file_hashes__relatedList
    )
    phantom.debug(
        "Filter_on_RelatedHash_and_count resultCount: %d"
        % extract_file_hashes__resultCount
    )
    ...
    phantom.save_run_data(
        key="extract_file_hashes:relatedList",
        value=json.dumps(extract_file_hashes__relatedList),
    )
    phantom.save_run_data(
        key="extract_file_hashes:resultCount",
        value=json.dumps(extract_file_hashes__resultCount),
    )
...
def file_hash_search_string(
    ...
    template = (
        """index=* sourcetype=\"symantec:ep:risk:file\" Application_Hash IN ({0})"""
    )
...
def no_related_entities_comment(
    action=None,
    success=None,
    container=None,
    results=None,
    ...
    phantom.comment(
        container=container, comment="Searches for Related Entities yielded no results."
    )
...
def join_check_for_results(
    ...
    if phantom.actions_done(
        [
            "spunksearch_for_domains",
            "splunksearch_for_files",
            "spunksearch_for_vulns",
            "splunksearch_for_ip",
        ]
    ):
...
def only_if_results(
    ...
    # call connected blocks if condition 1 matched
    if matched_artifacts_1 or matched_results_1:
        recorded_future_threat_hunting_block_ip(
            action=action,
            success=success,
            container=container,
            results=results,
...
def send_message_2(
    ...
    phantom.act(
        "send message", parameters=parameters, assets=["slack"], name="send_message_2"
    )
