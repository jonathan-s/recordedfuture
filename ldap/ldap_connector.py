...
class LdapConnector(BaseConnector):
    ...
    def _process_empty_reponse(self, response, action_result):
        ...
        return RetVal(action_result.set_status(
            phantom.APP_ERROR,
            u"Empty response and no information in the header"), None)
    ...
    def _process_json_response(self, r, action_result):
        ...
        try:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR,
                                                   u"Unable to parse JSON response. Error: {0}".format(
                                                       str(e))), None)
    ...
    def _make_rest_call(self, endpoint, action_result, method="get", **kwargs):
        ...
        try:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR,
                                                   u"Error Connecting to server. Details: {0}".format(
                                                       str(e))), resp_json)
    ...
    def _handle_check_email(self, param):

        # Implement the handler here
        # use self.save_progress(...) to send progress messages
        ...
        # Add an action result object to self (BaseConnector) to represent the
        # action for this param
        action_result = self.add_action_result(ActionResult(dict(param)))
        ...
        email = param['email']
        self.save_progress('Analyzing fragment: %s' % email)

        # Attempt to split the fragment into an email address and
        ...
        # Do the search
        try:
            self.connection.search(config['user_base_dn'],
                                   '(employeenumber=%s)' % email_address,
                                   attributes=ldap3.ALL_ATTRIBUTES)
        except ldap3.core.exceptions.LDAPInvalidFilterError as err:
            self.save_progress(phantom.APP_ERROR,
                               message='Search failed: %s'
                                       % self.connection.last_error,
                               exception=err)
            action_result.get_status(phantom.APP_ERROR)
        except ldap3.core.exceptions.LDAPAttributeError as err:
            self.save_progress('Search failed: %s'
                               % self.connection.last_error,
                               exception=err)
            action_result.get_status(phantom.APP_ERROR)
        self.save_progress('Search successful, %d results.'
                           % len(self.connection.response))
    ...
    def handle_action(self, param):
        ...
        if action_id == 'test_connectivity':
            ...
            ret_val = self._handle_check_email(param)
    ...
    def initialize(self):
        ...
        self.connection = ldap3.Connection(config['ldap_servers'].split(','),
...
if __name__ == '__main__':
    ...
    if (username and password):
        try:
            ...
            print(
                "Unable to get session id from the platfrom. Error: " + str(e))
            exit(1)
