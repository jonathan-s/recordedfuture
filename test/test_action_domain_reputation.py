...
class RfDomainReputationTests(RfTests):
    ...
    def _test_domain_reputation_score(self, ioc, target_risk_score):
        ...
        container_id = self._create_event_and_artifact(
            "Test Event Domain Reputation", destinationDnsDomain=ioc
        )
    ...
    def test_domain_reputation(self):
        ...
        targets = self.high_risk_iocs_by_category(
            "domain", 5, fields=["entity", "risk"]
        )
    ...
    def test_neg_domain_reputation_not_existing(self):
        """Test behavior when a non-existing domain is passed."""

        testdata = {"destinationDnsDomain": "my.nonexistingdomain.nu"}

        # Create container and artifact.
        container_id = self._create_event_and_artifact(
            "Test Event Domain Reputation - not existing",
